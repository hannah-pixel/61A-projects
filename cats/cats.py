...
def about(subject):
    ...
    def paragraph(p):
        string = split(p)
        for s in string:
            new_string = remove_punctuation(s.lower())
            for sub in subject:
                if new_string == sub:
                    return True
        return False
...
def accuracy(typed, source):
    ...
    if len(typed) > len(source) or len(typed) < len(source):
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for i in range(len(word_list)):
        difference = diff_function(typed_word, word_list[i], limit)
        if difference <= limit:
            if difference < og_limit:
                og_limit = difference
                autocorrect_word = word_list[i]
...
def feline_fixes(typed, source, limit):
    ...
    if typed == source:
        ...
        return len(typed)
    elif (len(typed) < len(source)) and len(typed) == 0:
        return len(source) 
...
def minimum_mewtations(typed, source, limit):
    ...
    if typed != source and (len(typed) == 1 and len(source) == 1): # Feel free to remove or add additional cases
        # BEGIN
        return 1
        # END
    ...
    if typed == source: # Base cases should go here, you may add more base cases as needed.
        ...
        difference = typed[0] != source[0]
        add = 1 + minimum_mewtations(typed, source[1:], limit-1) # Fill in these lines
        remove = 1 + minimum_mewtations(typed[1:], source, limit-1)
        substitute = difference + minimum_mewtations(typed[1:], source[1:], limit-difference)
        # BEGIN
...
def time_per_word(words, timestamps_per_player):
    ...
    """
    # BEGIN PROBLEM 9
    times = [[abs(t[i]- t[i+1]) for i in range(len(t)-1)] for t in timestamps_per_player]
...
def fastest_words(match):
    ...
    fastest = [[] for i in all_times]

    for w in word_indices:
        fastest_player = 0
        fastest_time = time(match, 0, w)
        for p in player_indices:
            player_time = time(match, p, w)
            if player_time < fastest_time:
                fastest_time = player_time
                fastest_player = p
        fastest[fastest_player].append(get_word(match, w))
