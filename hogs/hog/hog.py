...
def boar_brawl(player_score, opponent_score):
    ...
    score = abs( (opponent_score // 10) - (player_score % 10) ) * 3
...
def is_always_roll(strategy, goal=GOAL):
    ...
    while (score0<=goal):
        ...
        while (score1<=goal):
            prescore1=score1
            score1+=1
            if (strategy(score0,score1)!=strategy(prescore0, prescore1)):
                return False
        score0+=1
    return True
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    # BEGIN PROBLEM 9
    n = 1
    max_avg = make_averaged(roll_dice, samples_count)(n, dice)
    max_n = 1
    while (n < 10):
        new_avg = make_averaged(roll_dice, samples_count)(n+1, dice)
        if (max_avg < new_avg):
            max_avg = new_avg
            max_n = n+1
        n += 1
